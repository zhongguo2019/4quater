...
public abstract class ServiceMybatis<T> implements BaseService<T> {
    ...
    public int insert(T record) {
        ...
        BeanUtils.setProperty(record, "delFlag", Constant.DEL_FLAG_NORMAL, false);
		/*
		 * SysUser sysUser = SysUserUtils.getCacheLoginUser(); if (sysUser != null) {
		 * BeanUtils.setProperty(record, "createBy",
		 * SysUserUtils.getCacheLoginUser().getId(), false); }
		 */
    ...
    public int updateByPrimaryKey(T record) {
		...
		/*
		 * SysUser sysUser = SysUserUtils.getCacheLoginUser(); if (sysUser != null) {
		 * BeanUtils.setProperty(record, "updateBy",
		 * SysUserUtils.getCacheLoginUser().getId(), false); }
		 */
        BeanUtils.setProperty(record, "updateDate", new Date(), false);
...
//			String tableName = StringConvert.camelhumpToUnderline(cl.getSimpleName());
    ...
    public List<T> findEntityListByDataScope(final T record) {
        ...
        String sql = new SQL() {{
            ...
            com.boot.util.mybatis.EntityHelper.EntityTable entityTable = EntityHelper.getEntityTable(entityClass);
            SELECT(com.boot.util.mybatis.EntityHelper.getAllColumns(entityClass));
    ...
    public static MetaObject forObject(Object object) {
        return MetaObject.forObject(object, new DefaultObjectFactory(), new DefaultObjectWrapperFactory(), null);
    }
